...
public class AlberoBinarioImpl implements AlberoBinario{
	...
	public int level(NodoBinario u) {
		...
		//caso induttivo: level(u) = level(padre(u))+1
	...
	public int altezza() {
		...
		else return altezza(radice); //metodo sotto
	};
	
	// versione privata così non viene chimaata dall'esterno ma solo da altezza() di AlberoBinarioImpl.java
	private int altezza(NodoBinario u) {
		/*
		Integer  hsx, hdx;
		hsx= 0; hdx=0;
		if (u.sinistro==null && u.destro==null) return 0; //foglia
		else if(u.sinistro!=null) hsx= altezza(u.sinistro);
		else if (u.destro!=null) hdx = altezza(u.destro);
		return hsx>hdx ? 1+hsx : 1+hdx;
		*/
		if(u==null) return -1;
		return Math.max(altezza(u.sinistro), altezza(u.destro)) +1;
		/* quindi quando arrivo ad una foglia avrò -1 per i figli perchè sono null, sommo 1 e ottengo 0 infatti sono foglie
		 * e poi salendo nei nodi sommerò sempre 1
		 * infine calcolerò il max tra il ramo dx e sx nel root, come fatto anche in tutti i nodi intermedi.
		 */
	}
	
	// numero dei nodi NON foglia, numero di nodi interni
	...
	private int numNodiInterni(NodoBinario u) {
		...
		return 1+nsx+ndx; //aggiungo anche il nodo stesso
	}
	...
	private boolean equalsNodo (NodoBinario u, NodoBinario v){
		...
		if ((u==null && v!=null) ||
		    (u!=null && v== null))
			{return false;} //uno dei due alberi Ã¨ vuoto
		if (u==null && v==null)
	...
	private void eliminaFoglieUguali (NodoBinario u) {
		...
		if (u.sinistro != null) {
			if (u.sinistro.sinistro!= null ||
				u.sinistro.destro!= null) // u.sinistro non Ã¨ una foglia
		...
		if (u.destro != null) {
			if (u.destro.sinistro!= null ||
				u.destro.destro!= null) // u.destro non Ã¨ una foglia
	...
	public AlberoBinario pota(NodoBinario rimosso){
		...
		if(rimosso.padre==null){ //"rimosso" ï¿½ la radice dell'albero
			radice = null;
			return new AlberoBinarioImpl(rimosso); //restituiamo l'intero albero
		}
		NodoBinario pad = rimosso.padre;
		if(figlioSinistro(rimosso)){ //se "rimosso" ï¿½ figlio sinistro 
